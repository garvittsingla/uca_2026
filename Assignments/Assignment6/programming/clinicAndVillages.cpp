...
int divideHospitals(int n, int k, int population[]) {
    ...
    //{density,currentHospitals,totalPopulation}

    int clinicsLeft = k;
    for(int i = 0 ; i < n ; i++){
        pq.push({population[i],1,population[i]});
        clinicsLeft--;
    }

    while(clinicsLeft > 0){
       int density = pq.top()[0];
       int currentHospitals = pq.top()[1];
       int totalPopulation = pq.top()[2];
       pq.pop();

       int newDensity = std::ceil((double)totalPopulation / (currentHospitals+1));

       pq.push({newDensity,currentHospitals+1,totalPopulation});
       clinicsLeft--;
    }

    return pq.top()[0];
}
// TestCases generated by Large Language Model to check the answer is correct or not
